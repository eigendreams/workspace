...
#define ACCEL_Y_MIN ((float) -284)
#define ACCEL_Y_MAX ((float) 300)
...
const float magn_ellipsoid_transform[3][3] = {{0.771573, -0.00651306, -0.0151888}, {-0.00651306, 0.791164, -0.0424512}, {-0.0151888, -0.0424512, 0.990528}};

// Gyroscope
// "gyro x,y,z (current/average) = .../OFFSET_X  .../OFFSET_Y  .../OFFSET_Z
#define GYRO_AVERAGE_OFFSET_X ((float) -8.74)
#define GYRO_AVERAGE_OFFSET_Y ((float) 38.98)
#define GYRO_AVERAGE_OFFSET_Z ((float) -5.45)
...
void output_angles();
void output_calibration(short calibration_sensor);
...
void IMU::GetAngle() {
  ...
  if (Serial.available() >= 2)
  {
    ...
    {
      short command = Serial.read(); // Commands
...
float IMU::GetRoll(float offset) {
	GetAngle();
	return (TO_DEG(roll) + offset);
}

float IMU::GetPitch(float offset) {
    GetAngle();
	return (TO_DEG(pitch) + offset);
}

float IMU::GetYaw(float offset) {
    GetAngle();
	return (TO_DEG(yaw) + offset);
}

float IMU::Gyro_0() {
	GetAngle();
	return GYRO_SCALED_RAD(gyro[0]);
}

float IMU::Gyro_1() {
	GetAngle();
	return GYRO_SCALED_RAD(gyro[1]);
}

float IMU::Gyro_2() {
	GetAngle();
	return GYRO_SCALED_RAD(gyro[2]);
}
...
void Read_Accel()
{
  ...
  if (i == 6)  // All bytes received?
  {
    ...
    accel[0] = (short)((((short) buff[3]) << 8) | buff[2]);  // X axis (internal sensor y axis)
    accel[1] = (short)((((short) buff[1]) << 8) | buff[0]);  // Y axis (internal sensor x axis)
    accel[2] = (short)((((short) buff[5]) << 8) | buff[4]);  // Z axis (internal sensor z axis)
  }
  else
  {
    num_accel_errors++;
...
// 9DOF Razor IMU SEN-10125 using HMC5843 magnetometer
    ...
    magnetom[0] = -1 * (short)((((short) buff[2]) << 8) | buff[3]);  // X axis (internal sensor -y axis)
    magnetom[1] = -1 * (short)((((short) buff[0]) << 8) | buff[1]);  // Y axis (internal sensor -x axis)
    magnetom[2] = -1 * (short)((((short) buff[4]) << 8) | buff[5]);  // Z axis (internal sensor -z axis)
// 9DOF Razor IMU SEN-10736 using HMC5883L magnetometer
#elif HW__VERSION_CODE == 10736
    // MSB byte first, then LSB; Y and Z reversed: X, Z, Y
    magnetom[0] = -1 * (short)((((short) buff[4]) << 8) | buff[5]);  // X axis (internal sensor -y axis)
    magnetom[1] = -1 * (short)((((short) buff[0]) << 8) | buff[1]);  // Y axis (internal sensor -x axis)
    magnetom[2] = -1 * (short)((((short) buff[2]) << 8) | buff[3]);  // Z axis (internal sensor -z axis)
// 9DOF Sensor Stick SEN-10183 and SEN-10321 using HMC5843 magnetometer
#elif (HW__VERSION_CODE == 10183) || (HW__VERSION_CODE == 10321)
    // MSB byte first, then LSB; X, Y, Z
    magnetom[0] = (short)((((short) buff[0]) << 8) | buff[1]);         // X axis (internal sensor x axis)
    magnetom[1] = -1 *(short)((((short) buff[2]) << 8) | buff[3]);  // Y axis (internal sensor -y axis)
    magnetom[2] = -1 * (short)((((short) buff[4]) << 8) | buff[5]);  // Z axis (internal sensor -z axis)
// 9DOF Sensor Stick SEN-10724 using HMC5883L magnetometer
#elif HW__VERSION_CODE == 10724
    // MSB byte first, then LSB; Y and Z reversed: X, Z, Y
    magnetom[0] = (short)((((short) buff[0]) << 8) | buff[1]);         // X axis (internal sensor x axis)
    magnetom[1] = -1 * (short)((((short) buff[4]) << 8) | buff[5]);  // Y axis (internal sensor -y axis)
    magnetom[2] = -1 * (short)((((short) buff[2]) << 8) | buff[3]);  // Z axis (internal sensor -z axis)
#endif
  }
  else
...
void Read_Gyro()
{
  ...
  if (i == 6)  // All bytes received?
  {
    gyro[0] = -1 * (short)((((int) buff[2]) << 8) | buff[3]);    // X axis (internal sensor -y axis)
    gyro[1] = -1 * (short)((((int) buff[0]) << 8) | buff[1]);    // Y axis (internal sensor -x axis)
    gyro[2] = -1 * (short)((((int) buff[4]) << 8) | buff[5]);    // Z axis (internal sensor -z axis)
  }
  else
  {
    num_gyro_errors++;
