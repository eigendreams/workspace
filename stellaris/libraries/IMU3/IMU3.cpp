...
/**************************************************/
void IMU3::Drift_correction(void)
{
  float mag_heading_x;
  float mag_heading_y;
  float errorCourse;
...
float IMU3::Vector_Dot_Product(const float v1[3], const float v2[3])
{
  float result = 0;
...
void IMU3::Vector_Cross_Product(float out[3], const float v1[3], const float v2[3])
{
...
void IMU3::Vector_Scale(float out[3], const float v[3], float scale)
{
  for(short c = 0; c < 3; c++)
  {
    out[c] = v[c] * scale; 
...
// Adds two vectors
void IMU3::Vector_Add(float out[3], const float v1[3], const float v2[3])
{
  for(short c = 0; c < 3; c++)
  {
    out[c] = v1[c] + v2[c];
...
void IMU3::Matrix_Multiply(const float a[3][3], const float b[3][3], float out[3][3])
{
...
void IMU3::Matrix_Vector_Multiply(const float a[3][3], const float b[3], float out[3])
{
  for(short x = 0; x < 3; x++)
  {
...
void IMU3::init_rotation_matrix(float m[3][3], float yaw, float pitch, float roll)
{
  float c1 = cos(roll);
  float s1 = sin(roll);
  float c2 = cos(pitch);
...
void IMU3::Accel_Init()
{
  Wire3.beginTransmission(ACCEL_ADDRESS);
...
void IMU3::Read_Accel()
{
  ...
  Wire3.beginTransmission(ACCEL_ADDRESS);
  Wire3.requestFrom(ACCEL_ADDRESS, 6);  // Request 6 bytes
  while(Wire3.available())  // ((Wire3.available())&&(i<6))
  { 
...
void IMU3::Magn_Init()
{
  Wire3.beginTransmission(MAGN_ADDRESS);
  WIRE_SEND(0x02); 
...
void IMU3::Read_Magn()
{
  ...
  Wire3.beginTransmission(MAGN_ADDRESS); 
  Wire3.requestFrom(MAGN_ADDRESS, 6);  // Request 6 bytes
  while(Wire3.available())  // ((Wire3.available())&&(i<6))
  { 
    buff[i] = WIRE_RECEIVE();  // Read one byte
    i++;
...
void IMU3::Gyro_Init()
{
  ...
  Wire3.beginTransmission(GYRO_ADDRESS);
  WIRE_SEND(0x3E);
  WIRE_SEND(0x80);
  Wire3.endTransmission();
  delay(5);
...
void IMU3::Read_Gyro()
{
  ...
  Wire3.beginTransmission(GYRO_ADDRESS);
  Wire3.requestFrom(GYRO_ADDRESS, 6);  // Request 6 bytes
  while(Wire3.available())  // ((Wire3.available())&&(i<6))
  { 
    buff[i] = WIRE_RECEIVE();  // Read one byte
    i++;
...
void IMU3::compensate_sensor_errors() {
...
void IMU3::check_reset_calibration_session()
{
...
void IMU3::turn_output_stream_on()
{
  output_stream_on = true;
  digitalWrite(STATUS_LED_PIN, HIGH);
}

void IMU3::turn_output_stream_off()
{
