...
public:
	Servo servo;
	TalonClass(uint8_t umbral, int8_t sense);									// Associates the Talon with a Servo attach pin
	TalonClass(int min, int max, uint8_t umbral, int8_t sense);					// Associates the Talon with a Servo attach pin and sets min and max values in uS (RECALCULATES CENTER!)
	TalonClass(int min, int max, int center, uint8_t umbral, int8_t sense);		// Also sets the center value
