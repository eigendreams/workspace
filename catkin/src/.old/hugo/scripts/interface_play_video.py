...
class Interface_Play_Video:
    
    def __init__(self, node_name_override = 'interface_play_video'):
        ...
        rospy.loginfo("interface_play_video starting with name %s", self.nodename) 
        ...
        self.freq = 5
        self.rate = rospy.get_param("interface_play_video_rate", self.freq)
        ...
        self.videoSub = rospy.Subscriber("video", Int16, self.videoCb)
    ...
    def videoCb(self, data):
        if (data.data > 0):
            ...
            subprocess.call(["omxplayer", "-o", "hdmi", '/home/pi/video/video' + str(data.data) + '.mp4' ])
...
if __name__ == '__main__':
    """ main """
    interface_play_video = Interface_Play_Video()
    interface_play_video.spin()
